...
// Alternate screen buffer (like vim/less/htop): a separate, blank screen with
// NO scrollback — the user can't scroll up into stale frames, and on exit the
// terminal's original contents are restored. Combined with home-anchored
// rendering in liveMenu, this keeps the UI pinned to the top and stable.
 ...
 * no background tick, so we render with ANSI — each frame homes the cursor to
 * the top-left of the (alternate) screen and clears downward, so the UI is
 * pinned to the top and nothing scrolls or drifts.
...
async function liveMenu(message: string, options: MenuOption[]): Promise<MenuResult> {
  ...
  const build = (): string[] => {
    ...
    if (onJob) {
      ...
      if (logScroll > 0 && lastTotal >= 0 && lineCount > lastTotal) {
        logScroll += lineCount - lastTotal;
      }
      lastTotal = lineCount;
      ...
      for (let k = body.length; k < LOG_HEIGHT; k++) {
  ...
  const render = () => {
    const lines = build();
    // Home the cursor to the top-left, clear everything below, then draw. This
    // pins the frame to the top of the screen regardless of any output (clack
    // prompts, logs) printed between menus, so the UI never drifts down.
    Deno.stdout.writeSync(encoder.encode(`\x1b[H\x1b[0J${lines.join("\n")}`));
  };
...
async function main() {
  ...
  // Enter the alternate screen so the wizard owns a fresh, scroll-locked screen
  // and the user's prior terminal contents are restored on exit.
  Deno.stdout.writeSync(encoder.encode(ALT_SCREEN_ON));
  console.clear();
  ...
  try {
    while (true) {
      const result = await liveMenu("What do you want to run?", [
        ...
        },
        {
          ...
          hint: "remove done/failed/stopped",
        },
    ...
    // Always restore the user's original terminal screen, even on error.
    Deno.stdout.writeSync(encoder.encode(ALT_SCREEN_OFF));
  }
  p.outro("Bye 👋");
  Deno.exit(0);
}
