...
if git rev-parse --git-dir > /dev/null 2>&1; then
  ...
  git_str="$branch"
  [ "$staged" -gt 0 ] && git_str="${git_str} $(printf "${GREEN}+%s${RESET}" "$staged")"
  [ "$modified" -gt 0 ] && git_str="${git_str} $(printf "${YELLOW}~%s${RESET}" "$modified")"
else
  git_str="no branch"
fi
...
format_rl() {
  ...
  bar=$(make_bar "$pct")
  printf "%s%s %s %s%% resets %s%s" "$color" "$label" "$bar" "$pct" "$reset_time" "$RESET"
}
rate_limit_str=""
