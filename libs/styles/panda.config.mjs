...
//
// This config lives in the `styles` library (`libs/styles`), the single home for
// the design system: tokens, global CSS, and the generated `styled-system/`. It is
// still loaded from the workspace root via a 1-line shim (the root `panda.config.mjs`
// re-exports this file), so Panda keeps resolving the root-relative `include` globs
// and `outdir` below, and every `panda` CLI invocation across deno.json / project.json
// works unchanged.
//
// One config scans the app and the libraries, and emits a single `styled-system/`
// (aliased as `@zero-app/styled-system`) that all three packages import from. This
// follows the "include src files" approach from the Panda component-library guide
// (https://panda-css.com/docs/guides/component-library): because the app bundles the
// libraries from source (via Vite aliases), one Panda codegen covers app + libs.
//
...
export default {
  ...
  theme: {
    extend: {
      ...
      // defined in `./src/tokens.css` (`:root` for light, `.dark`
